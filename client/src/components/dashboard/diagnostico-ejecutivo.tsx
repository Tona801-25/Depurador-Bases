          ...
          <CardContent className="space-y-4 p-4 md:p-5">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <h3 className="text-lg font-bold text-foreground">
                  Bases, prefijos y franjas con mayor impacto comercial
                </h3>

                <p className="mt-1 max-w-4xl text-sm leading-relaxed text-muted-foreground">
                  {data.comparativaMultiarchivo.recomendacionGeneral}
                </p>
              </div>

              <Badge className="border-primary/25 bg-primary/15 text-primary">
                {data.comparativaMultiarchivo.totalArchivos} archivo
                {data.comparativaMultiarchivo.totalArchivos !== 1 ? "s" : ""}
              </Badge>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div className="rounded-xl border border-border bg-background/60 p-3">
                ...
                <p className="text-xl font-bold text-foreground">
                  {data.comparativaMultiarchivo.basesRepetidas.length}
                </p>

                <p className="mt-1 text-xs text-muted-foreground">
                  Bases presentes en más de un archivo cargado.
                </p>
              </div>
              ...
              <div className="rounded-xl border border-border bg-background/60 p-3">
                ...
                <p className="text-xl font-bold text-foreground">
                  {data.comparativaMultiarchivo.prefijoMasEstable || "-"}
                </p>
              ...
              <div className="rounded-xl border border-border bg-background/60 p-3">
                ...
                <p className="text-xl font-bold text-foreground">
                  {data.comparativaMultiarchivo.franjaMasConveniente || "-"}
                </p>

                <p className="mt-1 text-xs text-muted-foreground">
                  Horario con mejor contacto efectivo relativo.
                </p>
              </div>
            </div>

            {data.comparativaMultiarchivo.basesRepetidas.length > 0 && (
              <div className="rounded-xl border border-warning/20 bg-warning/5 p-3">
                ...
                <div className="space-y-2">
                  {data.comparativaMultiarchivo.basesRepetidas.slice(0, 3).map((base) => (
                    <div
                      key={base.base}
            ...
            {data.comparativaMultiarchivo.mejoresSegmentos.length > 0 && (
              <div className="rounded-xl border border-success/20 bg-success/5 p-3">
                ...
                <div className="grid grid-cols-1 gap-2 lg:grid-cols-2">
                  {data.comparativaMultiarchivo.mejoresSegmentos.slice(0, 4).map((segmento) => (
                    <div
            ...
            {data.comparativaMultiarchivo.segmentosARevisar.length > 0 && (
              <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-3">
                ...
                <div className="grid grid-cols-1 gap-2 lg:grid-cols-2">
                  {data.comparativaMultiarchivo.segmentosARevisar.slice(0, 4).map((segmento) => (
                    <div
                      key={`${segmento.archivoOrigen}-${segmento.base}-${segmento.prefijo}-${segmento.franja}`}
                      className="rounded-lg border border-border/70 bg-background/60 p-3"
                    >
